function markTripletFreqDuringGazeAsStruct(tripletsPath,eventsPath,shuffPath)
 ...
 markedTriplets = struct();
 mtc = 0;

 [saveDir,saveStr,~] = fileparts(tripletsPath);


saveStr = strrep(saveStr, 'triplets', 'marked');
 savePath = fullfile(saveDir,[saveStr,'.mat']);
    ...
    for u = 1:numUniqueLabels
        ...
        for tp = 1:numTripletPerms
            ...
            %if shuffsGreaterThan >= 95 || shuffsGreaterThan <= 5
               ...
               mtc = mtc+1;
                markedTriplets(mtc).IDStr = IDStr;
                markedTriplets(mtc).regionStr = regionStr;
                markedTriplets(mtc).c1_UUID = c1_UUID;
                markedTriplets(mtc).c2_UUID = c2_UUID;
                markedTriplets(mtc).c3_UUID = c3_UUID;
                markedTriplets(mtc).gazeLabel = gazeLabel;
                markedTriplets(mtc).tp = tp;
                markedTriplets(mtc).shuffsGreaterThan = shuffsGreaterThan;
                markedTriplets(mtc).periodStr = 'pre';
            %end
          ...
          shuffsGreaterThan = numel(find(yv>xVals(tp,:)));
           ...
           % if shuffsGreaterThan >= 95 || shuffsGreaterThan <= 5
               ...
               % markTriplets(IDStr, regionStr, c1_UUID, c2_UUID, c3_UUID, gazeLabel, tp, shuffsGreaterThan, 'post')
                ...
                markedTriplets(mtc).tp = tp;
                markedTriplets(mtc).shuffsGreaterThan = shuffsGreaterThan;
                markedTriplets(mtc).periodStr = 'post';
...
markedTripletTable = struct2table(markedTriplets);
   save(savePath, 'markedTriplets', '-v7.3');
