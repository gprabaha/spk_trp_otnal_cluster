...
for c = 1:size(triplets,2)
 ...
 if exist('markedTriplets.mat', 'file')
     
     load('markedTriplets.mat', 'markedTrips')
     
     i = size(markedTrips,2)+1;
 ...
 markedTrips(i).IDStr = IDStr;
  markedTrips(i).regionStr = regionStr;   
 markedTrips(i).c1_UUID = c1_UUID;   
  markedTrips(i).c2_UUID = c2_UUID;   
   markedTrips(i).c3_UUID = c3_UUID;   
   markedTrips(i).gazeLabel = gazeLabel;
    markedTrips(i).tripPerm = tp;
    markedTrips(i).shuffsGreaterThan = shuffsGreaterThan;
    markedTrips(i).periodStr = periodStr;
   
   save('markedTriplets.mat', 'markedTrips')
   
  
  %C = {'NT', IDStr, regionStr, c1_UUID, c2_UUID, c3_UUID, gazeLabel, tp, shuffsGreaterThan, periodStr};
  %writecell(C,'trips.csv', 'WriteMode', 'append')
 end